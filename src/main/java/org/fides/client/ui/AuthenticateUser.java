...
public class AuthenticateUser {
	...
	private static boolean authenticateUserWithUi(ServerConnector serverConnector) {
		...
		while (option == LOGIN || option == REGISTER || option == CANCEL) {
			...
			Boolean authenticated = execute(option, errorMessages, serverConnector, usernameString, passwordHashString);
	...
	private static boolean authenticateUserFromConfig(ServerConnector serverConnector) {
		String usernameString = UserProperties.getInstance().getUsername();
	...
	private static boolean execute(int option, ArrayList<String> errorMessages, ServerConnector serverConnector, String usernameString, String passwordHashString) {
		...
		if (errorMessages.isEmpty()) {
			if (option == LOGIN) {
				if (serverConnector.login(usernameString, passwordHashString)) {
					log.debug("login successful");
					UserProperties.getInstance().setPasswordHash(passwordHashString);
					UserProperties.getInstance().setUsername(usernameString);
					return true;
				} else {
			...
			if (option == REGISTER) {
				if (serverConnector.register(usernameString, passwordHashString)) {
